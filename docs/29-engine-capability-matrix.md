...
| 🔧¹⁰ | `scripts/patch-wwebjs-media-id.js`                         | whatsapp-web.js `Injected/Utils.js`  | In WhatsApp Web build 2.3000.1047775310+ (September 2026), `mediaOptions` returned by `processMediaData` carries an internal `__x_id` property. Spreading `mediaOptions` into the outgoing `message` object causes `Msg.initialize` to fail on `getValidatedSender` with `Error: Data passed to getter must include an id property (it's how we memoize) but got undefined`. Deletes `message.__x_id` right after the outgoing message object is constructed, ensuring clean initialization with `newMsgKey` (upstream PR #201923). |
